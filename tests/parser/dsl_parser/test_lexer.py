...
def test_add_token_to_list():
    ...
    assert lexer.tokenList[0] == Token(Position('',1,1),'STRING','babyfoot')
...
def test_run_lexer():
    ...
    expectedOutput = [
        ...
        Token(Position('file2',1,26),'STRING','2'),
        Token(Position('file2',1,27),'EOF')
    ]
    lexer = Lexer(input)
    output = lexer.run()
